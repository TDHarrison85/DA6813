...
author: "Will Hyltin, Holly Milazo and Tim Harrison"
date: today
format: 
  html:
...
We normalized data to perform SVM and Logistic Regression but leave it raw for Random Forrest.
...
Using all variables to do this
...
1. Classify -aquire/not aquire all models types
2. Speak to limitations pro/cons of each model. Cover assumptions
2. Build duration fom susbset of customers we will acquire using best model available(probably random forrest)
...
Confusion Matrix for each classification model, cover sensitivity, specificity and comparison RF works better because
Pick best model and ran confusion matrix on full data set.

Variable importance plot 
retention expenditure is not linear relationship with retention
Show retention results

Performance Metrics like MAPE
3. PDP plot for extra credit
