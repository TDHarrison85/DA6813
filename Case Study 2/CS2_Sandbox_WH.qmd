...
raw_train <- read_xlsx(here('DA6813', 'Case Study 2', 'BBBC-Train.xlsx'))
raw_test <- read_xlsx(here('DA6813', 'Case Study 2', 'BBBC-Test.xlsx'))
```
...
```{r}
predprob_log <- predict(logfit, newdata = test1, type = "response")
pr_class_log <- ifelse(predprob_log > 0.5, 1, 0)
...
pr_class_log_imbal <- ifelse(predprob_log_imbal > 0.35, 1, 0)
...
```{r}
summary_table_log <- summ_tab_fun(combined, 50000, log_CM_imbal)
summary_table_log
```
...
```{r}
for (i in seq(0, .22, by = 0.01)) {
  preds <- ifelse(predprob_log_imbal > i, 1, 0)
  ...
  print(paste(i, sum(summ_for$profit)))
}
```
```{r}
CM_for
