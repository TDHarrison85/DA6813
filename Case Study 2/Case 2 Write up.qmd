...
df_train1 <- read_xlsx(here('Case Study 2', 'BBBC-Train.xlsx'))
df_test1 <- read_xlsx(here('Case Study 2', 'BBBC-Test.xlsx'))
```
...
df_combined <- rbind(df_train1, df_test1)
```
...
```{r}
df_combined %>% select_if(is.numeric) %>% cor() %>% corrplot(method = 'number')
```
...
```{r}
df_thresh <- data.frame(threshold = -0.01, profit = 0)
for (i in seq(0, 1, by = 0.01)) {
  df_preds <- ifelse(predprob_log >= i, 1, 0)
  df_CM_for <- caret::confusionMatrix(as.factor(preds), as.factor(test1$Choice), positive = '1')
  df_summ_for <- summ_tab_fun(combined, 50000, CM_for)
  df_thresh = rbind(df_thresh, data.frame(threshold = i, profit = sum(df_summ_for$profit)))
}

df_thresh <- df_thresh %>% filter(threshold >= 0)
df_thresh
```
